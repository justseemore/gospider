...
// http.Header returns h. It exists so FrameHeaders can be embedded in other
...
// A http.HeadersFrame is used to open a stream and additionally carries a
...
// http.HeadersFrameParam are the parameters for writing a HEADERS frame.
type http2HeadersFrameParam struct {
	// StreamID is the required Stream ID to initiate.
	StreamID uint32
	// BlockFragment is part (or all) of a http.Header Block.
	BlockFragment []byte
...
//
// As with http.Header Frames, This is the low level call for writing
...
func http2ConfigureServer(s *http.Server, conf *http2Server) error {
	...
	protoHandler := func(hs *http.Server, c *tls.Conn, h http.Handler) {
		...
		// method on the http.Handler. This is for internal
...
type http2ServeConnOpts struct {
	...
	// http.Handler specifies which handler to use for processing
...
// the Request.TLS field in http.Handlers.
//
...
// *responseWriter{,State} itself, as the http.Handler ending nils out the
...
type http2stream struct {
	...
	flow             http2flow // limits writing from http.Handler to client
...
// setConnState calls the net/http http.ConnState hook for this connection, if configured.
...
func (sc *http2serverConn) readPreface() error {
	...
	timer := time.NewTimer(http2prefaceTimeout) // TODO: configurable on *http.Server?
	defer timer.Stop()
	select {
	case <-timer.C:
...
func (sc *http2serverConn) closeStream(st *http2stream, err error) {
	...
	st.closeErr = err
	st.cw.Close() // signals http.Handler's CloseNotifier, unblocks writes, etc
...
// copyTrailersToHandlerRequest is run in the http.Handler's goroutine in
...
func http2handleHeaderListTooLong(w http.ResponseWriter, r *http.Request) {
	// 10.5.1 Limits on http.Header Block Size:
	// .. "A server that receives a larger header block than it is
	// willing to handle can send an HTTP 431 (Request http.Header Fields Too
	// Large) status code"
	const statusRequestHeaderFieldsTooLarge = 431 // only in Go 1.6+
	w.WriteHeader(statusRequestHeaderFieldsTooLarge)
	io.WriteString(w, "<h1>HTTP Error 431</h1><p>Request http.Header Field(s) Too Large</p>")
}
...
// requestBody is the http.Handler's Request.Body type.
...
// http.Header), and because we didn't want to expand the ResponseWriter
// interface, and because nobody used trailers, and because RFC 7230
// says you SHOULD (but not must) predeclare any trailers in the
// header, the official ResponseWriter rules said trailers in Go must
// be predeclared, and then we reuse the same ResponseWriter.Header()
// map to mean both http.Headers and Trailers. When it's time to write the
// Trailers, we pick out the fields of http.Headers that were declared as
// trailers. That worked for a while, until we found the first major
// user of Trailers in the wild: gRPC (using them only over http2),
// and gRPC libraries permit setting trailers mid-stream without
// predeclaring them. So: change of plans. We still permit the old
// way, but we also permit this hack: if a http.Header() key begins with
// "Trailer:", the suffix of that key is a Trailer. Because ':' is an
// invalid token byte anyway, there is no ambiguity. (And it's already
// filtered out) It's mildly hacky, but not terrible.
//
// This method runs after the http.Handler is done and promotes any http.Header
...
func (w *http2responseWriter) FlushError() error {
	rws := w.rws
	if rws == nil {
		panic("Header called after http.Handler finished")
	}
	var err error
...
func (w *http2responseWriter) CloseNotify() <-chan bool {
	rws := w.rws
	if rws == nil {
		panic("CloseNotify called after http.Handler finished")
	}
...
func (w *http2responseWriter) WriteHeader(code int) {
	rws := w.rws
	if rws == nil {
		panic("WriteHeader called after http.Handler finished")
	}
...
//
// * http.Handler calls w.Write or w.WriteString ->
// * -> rws.bw (*bufio.Writer) ->
...
func (w *http2responseWriter) write(lenData int, dataB []byte, dataS string) (n int, err error) {
	rws := w.rws
	if rws == nil {
		panic("Write called after http.Handler finished")
	}
	if !rws.wroteHeader {
		w.WriteHeader(200)
	}
...
func http2shouldRetryRequest(req *http.Request, err error) (*http.Request, error) {
	...
	// If the Body is nil (or http.http.NoBody), it's safe to reuse
...
// per RFC 7540 section 8.1.2.2: Connection-Specific http.Header Fields.
...
func (cc *http2ClientConn) encodeHeaders(req *http.Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	enumerateHeaders := func(f2 func(name, value string)) {
		...
		for k, vv := range req.Header {
			if http2asciiEqualFold(k, "host") || http2asciiEqualFold(k, "content-length") {
				...
				// Per 8.1.2.2 Connection-Specific http.Header
	...
	// http.Header list size is ok. Write the headers.
	enumerateHeaders(func(name, value string) {
...
func (b http2transportResponseBody) Read(p []byte) (n int, err error) {
	...
	if v := cc.inflow.available(); v < int32(b.cs.cc.t.h2Ja3Spec.ConnFlow)/2 {
		connAdd = int32(b.cs.cc.t.h2Ja3Spec.ConnFlow) - v
	...
	if err == nil { // No need to refresh if the stream is over or failed.
		...
		if v < int(cs.cc.t.streamFlow)-http2transportDefaultStreamMinRefresh {
			streamAdd = int32(int(cs.cc.t.streamFlow) - v)
...
type http2writeContext interface {
	...
	Flush() error
	CloseConn() error
	// http.HeaderEncoder returns an HPACK encoder that writes to the
	// returned buffer.
...
func http2encodeHeaders(enc *hpack.Encoder, h http.Header, keys []string) {
	...
	for _, k := range keys {
		...
		for _, v := range vv {
			...
			// TODO: more of "8.1.2.2 Connection-Specific http.Header Fields"
			if isTE && v != "trailers" {
				continue
			}
			http2encKV(enc, k, v)
